...
- Record resolved items in
  `/project-management/state/bugs/resolved-bugs.txt`, adding the newest
  items to the top, preserving the formatting, and including the completion
  timestamp, `Root cause:` analysis, and concise `Resolution:` notes.
...
- Before running local checks in a fresh checkout or environment, bootstrap
  ...
  inside the checkout, installs required system tools when needed, and
  installs the project with `interactive` and `dev` extras by default.
  Interactive POSIX installs report whether another `mdview` command appears
  on PATH before prompting whether arbitrary-directory `mdview` invocations
  should prefer the checkout-local copy. Non-interactive runs can override
  that choice with `./scripts/install_prerequisites.sh --mdview-command
  local` or `./scripts/install_prerequisites.sh --mdview-command system`.
  After bootstrap, run local checks with `.venv/bin/python` or an activated
  `.venv`.
...
2. Static sanity:
   ...
   - `.venv/bin/python scripts/run_tool_with_timeout.py entropy_check`
   - `.venv/bin/python scripts/run_tool_with_timeout.py entropy_tripwire_verify`
3. Tests:
...
- `.venv/bin/python scripts/run_tool_with_timeout.py windows_shims_linux`

For standardized Git operations, prefer:
- `.venv/bin/python scripts/git_standard_commit_push.py -m "<message>"`
  for commit/push (runs cached quality gate checks, including entropy
  tripwire verification, before push).
- `.venv/bin/python scripts/git_veteran_pull.py` for guarded pull
  operations.
