...
On Unix-like hosts, development mode validates the Python 3.9 bootstrap floor,
provisions pyenv CPython 3.14.6 as the development runtime, refreshes `.venv`,
and enables direnv-backed project activation. Existing user-owned pyenv
checkouts are reused without being pulled or switched.
...
When automation runs with an isolated Codex or Claude `HOME`, select the real
user scope explicitly:
```bash
./install.sh --mode dev --user-home /home/username
```
User-scoped installs fail closed in a recognized assistant home when this
option is omitted. Successful installs report the selected user home.
