...
def test_quality_gate_cache_ignores_repo_local_codex_state_and_package_lock(
    tmp_path: Path,
) -> None:
    repo = _make_fake_repo(tmp_path)
    ...
    root_package_lock = repo / "package-lock.json"
    root_package_lock.write_text('{"integrity": "token-one"}\n', encoding="utf-8")
    ...
    root_package_lock.write_text('{"integrity": "token-two"}\n', encoding="utf-8")
